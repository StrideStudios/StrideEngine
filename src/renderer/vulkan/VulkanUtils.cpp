...
VkDescriptorSetLayout SDescriptorLayoutBuilder::build(VkDevice inDevice, VkShaderStageFlags inShaderStages, void *pNext, VkDescriptorSetLayoutCreateFlags inFlags) {
	 for (auto& b : bindings) {
...
void SDescriptorAllocator::init(VkDevice inDevice, uint32_t inMaxSets, std::span<PoolSizeRatio> inPoolRatios) {
...
void SDescriptorAllocator::clear(VkDevice inDevice) {
	vkResetDescriptorPool(inDevice, mPool, 0);
}

void SDescriptorAllocator::destroy(VkDevice inDevice) {
	vkDestroyDescriptorPool(inDevice,mPool,nullptr);
}

VkDescriptorSet SDescriptorAllocator::allocate(VkDevice inDevice, VkDescriptorSetLayout inLayout) {
