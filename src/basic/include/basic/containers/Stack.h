...
struct TStack : TContainer<TType> {
	...
	virtual TType& get(size_t index) override {
		...
		errs("TStack does not support random access!");
	}
...
private:
	...
	virtual void reserve(size_t index) override {
		errs("TStack does not have contiguous memory!");
	}
	...
	struct IterableStack : std::stack<TType> {
		std::deque<TType>& getContainer() {
			return std::stack<TType>::c;
		}
	} m_Container;
};