...
public:
	...
	static void transitionImage(struct SCommandBuffer& inCmd, struct SImage_T* inImage, VkImageLayout inLayout);
...
inline uint32 getVkFormatSize(const VkFormat inFormat) {
	switch (inFormat) {
		...
		case VK_FORMAT_MAX_ENUM:
			astsNoEntry(); //Cant be bothered to add the various empty cases (they are not common)
	}
	astsNoEntry();
}