...
namespace CMD5Hashing {

	// TODO: For lack of a better option, rip this straight from unreal (hope they arent too mad)
	static uint32 getHash(const std::string& string) {
		uint32 Hash = 0;

		for (int32 i = 0; i < string.size(); ++i) {
			const char* original = string.c_str();
			const char c = (char)((unsigned char)(*original++) - ((uint32(*original++) - 'a' < 26u) << 5));
			uint16  B  = c;
			Hash     = (Hash >> 8 & 0x00FFFFFF) ^ HashTable[(Hash ^ B) & 0x000000FF];
			B        = c>>8;
			Hash     = (Hash >> 8 & 0x00FFFFFF) ^ HashTable[(Hash ^ B) & 0x000000FF];
		}
		return Hash;
	...
	uint32 getFileHash(const std::string& filePath){
